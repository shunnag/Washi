...
- 非準拠の text/html など、表示できない章へ移動した後に、前章の DOM から
  誤った検索位置・矩形・テキストアンカーを返す問題を修正する。
- EPUB の scrolled-doc / scrolled-continuous と roll の表示に対応する。
  横書き・縦書きのスクロール、章境界、位置復元、検索、census と画面サムネイルを
  同じ表示モデルで扱う。ページ割りエンジン版を 5 にし、古い実測値を無効化する。
...
- README 冒頭にクイックスタートを追加し、導入・SwiftUI・ファイル権限・
  読み込みと終了・検索と画像取得の DocC ガイドを整備した。

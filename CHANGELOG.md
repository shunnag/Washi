...
- 公開 EPUB コーパスのうち W3C の206件の SHA-256 を更新した。配布サイトの再生成で
  zip の付帯情報だけが変わり、各ファイルの中身は記録済みのリビジョンと同一である
  ことを確認した。あわせて、エントリの順序・名前・圧縮方式・中身から求める内容
  ダイジェストを全251件に記録した。付帯情報だけが変わった場合は通知して受け入れ、
  CI を止めない。中身や構造が変わった場合は従来どおり失敗させる。
